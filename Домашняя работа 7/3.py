...
class Cell:
    ...
    def __sub__(self, other):
        ...
        if result <= 0:
            print("Из первой клетки не может быть вычтена вторая!")
        else:
            return Cell(result)
    ...
    def make_order(self, num_cells_in_row):
        ...
        remainder = self.num_cells - num_cells_in_row * num_rows
        star_rows = [["*"] * num_cells_in_row] * num_rows + [["*"] * remainder]

        return '\n'.join(''.join(element for element in row) for row in star_rows)
